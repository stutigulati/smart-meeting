'use client';

import { useEffect, useState } from 'react';
import { useRouter, useSearchParams } from 'next/navigation';
import { Zap, Shield, Users, AlertCircle } from 'lucide-react';
import { authApi, getToken } from '@/lib/api';

export default function LoginPage() {
  const router = useRouter();
  const params = useSearchParams();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => {
    if (getToken()) {
      authApi.me()
        .then(() => router.replace('/dashboard'))
        .catch(() => {});
    }
    if (params.get('error') === 'no_token') {
      setError('Login failed. Please try again.');
    }
  }, []);

  const handleGoogleLogin = async () => {
    setLoading(true);
    setError('');
    try {
      const data = await authApi.googleLoginUrl();
      // backend returns auth_url
      window.location.href = data.auth_url;
    } catch {
      setError('Could not connect to server. Make sure the backend is running on port 8000.');
      setLoading(false);
    }
  };

  return (
    <div className="min-h-screen bg-bg flex">

      {/* Left panel */}
      <div className="hidden lg:flex flex-col justify-between w-1/2 bg-bg-panel border-r border-border p-12">
        <div className="flex items-center gap-3">
          <img src="/gog-logo.ico" alt="GOG Logo" className="w-10 h-10 rounded-lg object-contain" />
          <span className="font-bold text-lg text-text">GOG OMS</span>
        </div>

        <div className="space-y-8">
          <div>
            <h1 className="text-4xl font-bold text-text leading-tight">
              Meeting intelligence,<br />
              <span className="text-accent">redefined.</span>
            </h1>
            <p className="mt-4 text-text-muted text-lg leading-relaxed">
              AI-powered transcription, instant reports, and smart scheduling — all in one place.
            </p>
          </div>

          <div className="space-y-4">
            {[
              { icon: Zap,    title: 'AI Meeting Reports',  desc: 'Gemini generates summaries instantly' },
              { icon: Shield, title: 'Secure & Private',    desc: 'Your data stays in your own backend' },
              { icon: Users,  title: 'Team Collaboration',  desc: 'Shared meeting history and transcripts' },
            ].map(({ icon: Icon, title, desc }) => (
              <div key={title} className="flex items-start gap-4">
                <div className="w-9 h-9 rounded-lg bg-accent-muted flex items-center justify-center flex-shrink-0 mt-0.5">
                  <Icon className="w-4 h-4 text-accent" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-text">{title}</p>
                  <p className="text-xs text-text-dim mt-0.5">{desc}</p>
                </div>
              </div>
            ))}
          </div>
        </div>

        <p className="text-xs text-text-dim">
          © {new Date().getFullYear()} GOG Technologies. All rights reserved.
        </p>
      </div>

      {/* Right panel */}
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm space-y-8">

          <div className="lg:hidden flex items-center gap-3 justify-center">
            <img src="/gog-logo.ico" alt="GOG Logo" className="w-10 h-10 rounded-lg object-contain" />
            <span className="font-bold text-lg text-text">GOG OMS</span>
          </div>

          <div>
            <h2 className="text-2xl font-bold text-text">Welcome back</h2>
            <p className="text-text-muted text-sm mt-1">
              Sign in with your Google account to continue
            </p>
          </div>

          {error && (
            <div className="flex items-start gap-3 p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm">
              <AlertCircle className="w-4 h-4 mt-0.5 flex-shrink-0" />
              {error}
            </div>
          )}

          <button
            onClick={handleGoogleLogin}
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl
                       bg-bg-card border border-border hover:border-accent hover:bg-accent-muted
                       text-text font-medium transition-all duration-200
                       disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <div className="w-5 h-5 border-2 border-accent border-t-transparent rounded-full animate-spin" />
            ) : (
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
            )}
            {loading ? 'Redirecting to Google...' : 'Continue with Google'}
          </button>

          <p className="text-center text-xs text-text-dim">
            By signing in you agree to our Terms of Service.
          </p>
        </div>
      </div>
    </div>
  );
}